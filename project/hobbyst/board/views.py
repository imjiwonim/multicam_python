...
def post_write(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            post.save()
        # Post 생성 후 request.FILES.getlists("images")로ㅓ 전송된 이미지들을 순회하며 PostImage 객체를 생성한다.
...
#         comment.post = post
        ...
        return HttpResponseRedirect(f"/board/#post-{comment.post.id}")              # 교재에는 posts/feed/#post-{comment.post.id} 로 되있음 (헷갈림;posts/feeds > board? or board/home/ ?)
